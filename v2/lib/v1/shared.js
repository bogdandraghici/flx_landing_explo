/* Shared page chrome, ported from v1. Fonts + style.css are imported by the
   Next.js layout / globals.css, so the v1 CSS/font imports are dropped here.
   `reduceMotion` is SSR-guarded (false on the server) since this module is part
   of a "use client" graph and still evaluates during server rendering. */
...
/* ---------------------------------------------------------------------------
   THEME
   The initial theme is set before first paint by a tiny inline script in the
   layout <head> (no-flash). Here we build the nav toggle and broadcast changes
...
function initTheme() {
  ...
  if (!right) return;
  // guard against React re-mounts / double init inserting two toggles
  if (right.querySelector('.nav__theme')) return;
